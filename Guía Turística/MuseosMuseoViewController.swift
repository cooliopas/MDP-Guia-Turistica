...
class MuseosMuseoViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, MKMapViewDelegate, CLLocationManagerDelegate, SWRevealViewControllerDelegate {
	...
	override func viewDidLoad() {
		...
		restea("Museo","Detalle",["Token":"01234567890123456789012345678901","IdLugar":museo.id]) { (request, response, JSON, error) in
			...
			if error == nil, let info = JSON as? NSDictionary {
				...
				self.museo.info = NSAttributedString(string: "No fue posible cargar esta información.", attributes: [NSFontAttributeName:UIFont(name: "HelveticaNeue", size: 13.0)!])
				self.museo.observaciones = NSAttributedString(string: "No fue posible cargar las observaciones.", attributes: [NSFontAttributeName:UIFont(name: "HelveticaNeue", size: 13.0)!])
		...
		if museo.latitud != 0 {
			...
			let museoCoordinate = CLLocationCoordinate2DMake(museo.latitud,museo.longitud)
			let region = MKCoordinateRegionMakeWithDistance(museoCoordinate, 800, 800)
			
			let annotation = MKPointAnnotation()
			annotation.coordinate = museoCoordinate
			annotation.title = museo.nombre
			annotation.subtitle = "\(museo.calleNombre) \(museo.calleAltura)"
	...
	@IBAction func mapaComoLlegar() {
		...
		if !hayRed() {
			...
			if ubicacionActual != nil {
				...
				let destination = CLLocationCoordinate2DMake(museo.latitud, museo.longitud)
...
//						println(error)
	...
	func alertaLocalizacion() {
		...
		var alertController = UIAlertController (title: "Acceso a la localización", message: "Para mostrar la ruta hasta el museo, es necesario que permitas el acceso a la localización desde esta aplicación.\n\nPodes permitir el acceso desde \"Ajustes\".", preferredStyle: .Alert)
