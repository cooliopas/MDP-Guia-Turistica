	...
	override func viewDidLoad() {
		...
		restea("Inmobiliaria","Detalle",["Token":"01234567890123456789012345678901","IdLugar":inmobiliaria.id]) { (request, response, JSON, error) in
			...
			if error == nil, let info = JSON as? NSDictionary {
				...
				self.inmobiliaria.info = NSAttributedString(string: "No fue posible cargar esta información.", attributes: [NSFontAttributeName:UIFont(name: "HelveticaNeue", size: 13.0)!])
				self.inmobiliaria.observaciones = NSAttributedString(string: "No fue posible cargar las observaciones.", attributes: [NSFontAttributeName:UIFont(name: "HelveticaNeue", size: 13.0)!])
		...
		if inmobiliaria.latitud != 0 {
			...
			let inmobiliariaCoordinate = CLLocationCoordinate2DMake(inmobiliaria.latitud,inmobiliaria.longitud)
			...
			annotation.title = inmobiliaria.nombre
			annotation.subtitle = "\(inmobiliaria.calleNombre) \(inmobiliaria.calleAltura)"
	...
	@IBAction func mapaComoLlegar() {
		...
		if !hayRed() {
			...
			if ubicacionActual != nil {
				...
				let destination = CLLocationCoordinate2DMake(inmobiliaria.latitud, inmobiliaria.longitud)
	...
	func tableView(tableView: UITableView, heightForRowAtIndexPath indexPath: NSIndexPath) -> CGFloat {
		...
		if indexPath.row == 0 {
			...
			if indexPath.row == 2 {
				...
				texto = inmobiliaria.info
				
			} else {
				
				texto = inmobiliaria.observaciones
